...
void KalmanFilter::init(const cv::Rect2d &bbox) {
  ...
  P_wh.block<2, 2>(0, 0) =
      Eigen::Matrix2d::Identity(2, 2) * kf_param_.p_wh_size;
  P_wh.block<2, 2>(2, 2) = Eigen::Matrix2d::Identity(2, 2) * kf_param_.p_wh_ds;

  // initialize state
  x_xy = Eigen::Vector4d(bbox.x + 0.5 * bbox.width, bbox.y + 0.5 * bbox.height,
                         0, 0);
...
cv::Rect2d KalmanFilter::predict(const double dt) {
  ...
  // std::cout << "estimate: P = \n"
  //           << P << std::endl;
  return cv::Rect2d(x_xy(0) - x_wh(0) * 0.5, x_xy(1) - x_wh(1) * 0.5, x_wh(0),
                    x_wh(1));
}
...
cv::Rect2d KalmanFilter::update(const cv::Mat &T_mat, const double dt) {
  ...
  double f =
      sqrt(pow(T_mat.at<double>(0, 0), 2) + pow(T_mat.at<double>(0, 1), 2));
  ...
  // get measurement
  Eigen::Vector4d z_xy =
      Eigen::Vector4d(f, theta, tx, ty);             // z = [f, theat, tx, ty]
  ...
  // for clarity, we use the variable with the same name instead of the Eigen
  // index
  double x = x_xy(0);
  double y = x_xy(1);
  double vx = x_xy(2);
  ...
  // H_xy
  // first row
  H_xy(0, 0) = -(vx * dt * cos_theta + (y + vy * dt) * sin_theta -
                 tx * pow(cos_theta, 2) - ty * pow(sin_theta, 2)) /
               pow(x, 2);           // d(f)/d(x)
  ...
  // second row
  double a_xy =
      (pow(x, 2) + pow(y, 2)) * (x * vy * dt - vx * y * dt - x * ty + y * tx);
  double b_xy = (pow(x, 2) - pow(y, 2)) * (pow(x, 2) + x * vx * dt - x * tx +
                                           pow(y, 2) + y * vy * dt - y * ty) +
                1e-6;
  double alpha = a_xy / b_xy;
  double dalpha = 1 / (1 + pow(alpha, 2));  // d(arctan(alpha))/d(alpha)
  // d(theta)/d(x)
  double da_xy_dx = 3 * pow(x, 2) * vy * dt - 2 * x * vx * y * dt -
                    3 * pow(x, 2) * ty + 2 * x * y * tx + pow(y, 2) * vy * dt -
                    pow(y, 2) * ty;
  double db_xy_dx = 4 * pow(x, 3) + 3 * pow(x, 2) * vx * dt -
                    3 * pow(x, 2) * tx + 2 * x * y * vy * dt - 2 * x * y * ty -
                    pow(y, 2) * vx * dt + pow(y, 2) * tx;
  H_xy(1, 0) = dalpha * (da_xy_dx * b_xy - db_xy_dx * a_xy) / pow(b_xy, 2);
  // d(theta)/d(y)
  double da_xy_dy = -pow(x, 2) * vx * dt + pow(x, 2) * tx +
                    2 * y * x * vy * dt - 3 * vx * pow(y, 2) * dt -
                    2 * y * x * ty + 3 * pow(y, 2) * tx;
  double db_xy_dy = pow(x, 2) * vy * dt - pow(x, 2) * ty - 2 * x * y * vx * dt +
                    2 * x * y * tx - 4 * pow(y, 3) - 3 * pow(y, 2) * vy * dt -
                    3 * pow(y, 2) * ty;
  ...
  // H_wh
  // first row
  H_wh(0, 0) = -(vw * dt * cos_theta + (h + vh * dt) * sin_theta) /
               pow(w, 2);           // d(f)/d(w)
  ...
  double b_wh = (pow(w, 2) - pow(h, 2)) *
                (pow(w, 2) + w * vw * dt + pow(h, 2) + h * vh * dt);
  double gama = a_wh / b_wh;
  double dgama = 1 / (1 + pow(gama, 2));  // d(arctan(gama))/d(gama)
  // d(theta)/d(w)
  double da_wh_dw =
      3 * pow(w, 2) * vh * dt - 2 * w * vw * h * dt + pow(h, 2) * vh * dt;
  double db_wh_dw = 4 * pow(w, 3) + 3 * pow(w, 2) * vw * dt +
                    2 * w * h * vh * dt - 2 * x * y * ty - pow(h, 2) * vw * dt;
  H_wh(1, 0) = dgama * (da_wh_dw * b_wh - db_wh_dw * a_wh) / pow(b_wh, 2);
  // d(theta)/d(h)
  double da_wh_dh =
      -pow(w, 2) * vw * dt + 2 * h * w * vh * dt - 3 * vw * pow(h, 2) * dt;
  double db_wh_dh = pow(w, 2) * vh * dt - 2 * w * h * vw * dt - 4 * pow(h, 3) -
                    3 * pow(h, 2) * vh * dt;
  ...
  K_xy = P_xy * H_xy.transpose() *
         (H_xy * P_xy * H_xy.transpose() + R_xy).inverse();
  K_wh = P_wh * H_wh.transpose() *
         (H_wh * P_wh * H_wh.transpose() + R_wh).inverse();

  // std::cout << "K_xy: " << K_xy << std::endl;
  // std::cout << "K_wh： " << K_wh << std::endl;

  // calculate predicted measurement
  double f_predicted_xy =
      cos_theta + (vx * dt * cos_theta + (y + vy * dt) * sin_theta -
                   tx * pow(cos_theta, 2) - ty * pow(sin_theta, 2)) /
                      x;
  ...
  double f_predicted_wh =
      cos_theta + (vw * dt * cos_theta + (h + vh * dt) * sin_theta) / w;
  double theta_predicted_wh = atan(gama);

  // update x_xy, x_wh
  // std::cout << "x_xy before: " << x_xy << std::endl;
  // std::cout << "x_wh before: " << x_wh << std::endl;
  Eigen::Vector4d resiual_xy =
      z_xy - Eigen::Vector4d(f_predicted_xy, theta_predicted_xy,
                             tx_predicted_xy, ty_predicted_xy);
  Eigen::Vector2d resiual_wh =
      z_wh - Eigen::Vector2d(f_predicted_wh, theta_predicted_wh);
  // std::cout << "residual_xy: " << resiual_xy << std::endl;
  // std::cout << "residual_wh: " << resiual_wh << std::endl;

  // wrong measurement
  if (sqrt(pow(resiual_xy(2), 2) + pow(resiual_xy(3), 2)) >
      kf_param_.residual_threshold) {
