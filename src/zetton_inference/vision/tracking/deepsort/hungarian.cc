...
double HungarianAlgorithm::Solve(std::vector<std::vector<double>> &DistMatrix,
  ...
  auto *distMatrixIn = new double[static_cast<unsigned long>(nRows * nCols)];
  ...
  *cost = 0;
  ...
  } else /* if(nOfRows > nOfColumns) */
  {
    ...
    for (col = 0; col < nOfColumns; col++) {
      ...
      distMatrixTemp = distMatrix + static_cast<ptrdiff_t>(nOfRows * col);
  ...
  while (zerosFound) {
    ...
    for (col = 0; col < nOfColumns; col++)
      if (!coveredColumns[col])
        for (row = 0; row < nOfRows; row++)
          if ((!coveredRows[row]) &&
              (std::fabs(distMatrix[row + nOfRows * col]) < DBL_EPSILON)) {
            /* prime zero */
