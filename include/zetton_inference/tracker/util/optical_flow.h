...
class OpticalFlow {
 ...
 private:
  void detect_enough_keypoints(std::vector<LocalObject> &local_objects,
                               std::vector<cv::Point2f> &keypoints_all);
  void camera_motion_compensate(std::vector<cv::Point2f> &keypoints_all,
                                const std::vector<uchar> &status);
  void update_local_objects_curr_kp(std::vector<LocalObject> &local_objects,
                                    std::vector<cv::Point2f> &keypoints_all,
                                    const std::vector<uchar> &status);
  ...
  inline cv::Rect2d transform_bbox(const cv::Mat &H,
                                   const cv::Rect2d &bbox_pre);
  inline bool is_scene_points(const cv::Point2f &kp_pre,
